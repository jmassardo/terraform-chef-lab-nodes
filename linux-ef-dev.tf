...
resource "azurerm_virtual_machine" "linux-ef-dev" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo hab svc load ${var.hab_origin}/${var.audit_pkg_name} --channel dev --strategy at-once",
      "sudo hab svc load ${var.hab_origin}/${var.infra_pkg_name} --channel dev --strategy at-once",
    ]
  }
}
