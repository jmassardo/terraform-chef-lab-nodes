...
resource "azurerm_public_ip" "win-ef-dev-pubip" {
  name                = "win-ef-dev-pubip"
  location            = var.azure_region
  resource_group_name = azurerm_resource_group.rg.name
  allocation_method   = "Dynamic"
  domain_name_label   = "win-ef-dev-${lower(substr(join("", split(":", timestamp())), 8, -1))}"
...
resource "azurerm_network_interface" "win-ef-dev-ip" {
  name                = "win-ef-dev-ip"
  location            = var.azure_region
  resource_group_name = azurerm_resource_group.rg.name

  ip_configuration {
    name                          = "win-ef-dev-ipconf"
    subnet_id                     = azurerm_subnet.subnet.id
    private_ip_address_allocation = "dynamic"
    public_ip_address_id          = azurerm_public_ip.win-ef-dev-pubip.id
  }
}

#create the actual VM
resource "azurerm_virtual_machine" "win-ef-dev" {
  name                  = "win-ef-dev"
  location              = var.azure_region
  resource_group_name   = azurerm_resource_group.rg.name
  network_interface_ids = [azurerm_network_interface.win-ef-dev-ip.id]
  vm_size               = var.vm_size

  storage_os_disk {
    name              = "win-ef-dev-osdisk"
  ...
  connection {
    host     = azurerm_public_ip.win-ef-dev-pubip.fqdn
    type     = "winrm"
    port     = 5985
    https    = false
    timeout  = "60m"
...
output "win-ef-dev-fqdn" {
  value = azurerm_public_ip.win-ef-dev-pubip.fqdn
}
